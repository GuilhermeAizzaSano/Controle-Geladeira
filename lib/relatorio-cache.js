...
function createRelatorio({ pool, Q }) {
  ...
  function invalidateAndRefreshRelatorio() {
    // Invalida o cache de forma síncrona e marca o relatório como sujo. Até o REFRESH
    // assíncrono concluir, as leituras vão direto às tabelas (fallback), garantindo que
    // o admin sempre veja o estado real imediatamente após a ação.
    invalidarRelatorioCache();
    relatorioDirty = true;

    // Dispara o REFRESH da materialized view sem aguardar (fire-and-forget): o REFRESH varre
    // a tabela `consumo` inteira e ficaria mais lento conforme o banco cresce, então não deve
    // bloquear a resposta da compra/mutação. Ao concluir com sucesso, a MV volta a ser a fonte
    // das leituras; em caso de falha, mantém-se o fallback direto nas tabelas.
    Promise.resolve()
